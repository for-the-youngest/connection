...
export function registerComment(commentInfo, callback){
    fetch(`/v3/freeboard/${commentInfo.freeboardNumber}/reply`,
        {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({replyContent:commentInfo.content}),
        }).then(resp => {
...
export function getCommentList(freeboardNumber, callback){
    fetch(`/v3/freeboard/${freeboardNumber}/replys`, {
        method : 'get'
    }).then(resp => resp.json())
...
export function getCommentList2(freeboardNumber, page, callback){
    fetch(`/v4/freeboard/${freeboardNumber}/replys?page=${page}`, {
        method : 'get'
    }).then(resp => resp.json())
        .then(dataList => {
