...
function validateName() {
  ...
  if (inputName.length < 2 || inputName.length > 10) {
      document.getElementById('infoEdit-errMsgName').textContent = '2자 이상 10자 이하로 입력해야 합니다';
  ...
  if (!regex.test(inputName)) {
      document.getElementById('infoEdit-errMsgName').textContent = '이름은 한글 또는 영어만 포함해야 합니다';
      document.getElementById('infoEdit-errMsgName').style.display = 'block';
      return false;
  }
...
function validateNickName(){
  ...
  if(inputNickName.length < 2 || inputNickName.length > 10) {
    document.getElementById('infoEdit-errorMsgNkName').textContent = '2자 이상 10자 이하로 입력해야 합니다';
  ...
  if(inputNickName.includes(" ")) {
    document.getElementById('infoEdit-errorMsgNkName').textContent = '공백이 없어야 합니다';
    document.getElementById('infoEdit-errorMsgNkName').style.display = 'block';
    return false;
  }
  ...
  if(!Regex.test(inputNickName)) {
    document.getElementById('infoEdit-errorMsgNkName').textContent = '공백 없이 특수문자 "_"만 사용 가능합니다';
    document.getElementById('infoEdit-errorMsgNkName').style.display = 'block';
    return false;
  }
